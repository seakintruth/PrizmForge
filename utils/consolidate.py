        ...
        if isinstance(node, ast.ClassDef):
            ...
            for item in node.body:
                if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    symbols.append(
                        PySymbol("method", item.name, item.lineno, f"{q}.{item.name}")
                    )
        elif isinstance(node, ast.FunctionDef):
            symbols.append(
                PySymbol("function", node.name, node.lineno, f"{mod_name}.{node.name}")
            )
    ...
    for entry in files:
        ...
        tests = [
            ...
            and (
                ...
                or s.name.startswith("Test")
                or ".test_" in s.qualname
            )
        ]
...
def write_index_files(root_dir: str, report_dir: str, indexes: dict) -> dict:
    ...
    with combined_path.open("w", encoding="utf-8") as out:
        ...
        out.write(
            "Lightweight context files (no full source dump). "
            "Regenerate with `python utils/consolidate.py`.\n\n"
        )
        out.write("## Contents\n\n")
...
def consolidate_project(
    ...
    *,
    ...
    with open(output_filename, "w", encoding="utf-8") as outfile:
        ...
        outfile.write(
            "Generated by `utils/consolidate.py`. "
            "Indexes first, then full file dump.\n\n"
        )
        if index_paths:
            outfile.write("**Standalone indexes (preferred for LLM context):**\n\n")
            outfile.write(f"- Combined: `report/INDEX.md`\n")
            outfile.write(f"- Production: `report/index_production.md`\n")
            outfile.write(f"- Tests: `report/index_tests.md`\n")
            outfile.write(f"- Docs: `report/index_docs.md`\n\n")
...
def discover_project_root(start: str | None = None) -> str:
    ...
    4. cwd
    """
    sandbox_markers = ("/box", "/data/user", "/data/data")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Build code indexes and/or full consolidation report"
    )
    parser.add_argument(
        "--root",
        default=None,
