...
class PrioritizerWorker:
    ...
    def __init__(self):
        ...
        self.prioritization_interval = 20
        self.processing_cycle_time = 0
    ...
    def _filter_low_quality_feedback(self, items: list[FeedbackItem]) -> tuple[list[FeedbackItem], int]:
        ...
        for item in items:
            ...
            if not is_low_quality:
                if message_lower in ["issue here", "todo", "fix this"] and suggestion_lower in ["fix here", "todo", "fix this"]:
                    is_low_quality = True
                    reason = "Both message and suggestion are placeholders"
            if not is_low_quality and message_lower == item.category.lower():
                is_low_quality = True
                ...
                with get_db_connection() as conn:
                    for item, reason in dismissed_items:
                        if item.item_type == "feedback":
                            conn.execute(
                                "UPDATE agent_feedback SET addressed = 1, addressed_by = 'prioritizer_quality_filter', addressed_at = ? WHERE id = ?",
    ...
    def _should_run_cycle(self) -> bool:
        ...
        if self.last_prioritization is None:
            return True
        return (time.time() - self.last_prioritization) >= self.prioritization_interval
    ...
    def _get_all_feedback(self) -> list[FeedbackItem]:
        try:
            with get_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT id, agent_name, file_path, priority, category, message, suggestion, timestamp FROM agent_feedback WHERE task_id = ? AND addressed = 0 ORDER BY timestamp DESC",
                    (self.current_task_id,),
                )
                feedback_rows = cursor.fetchall()
                cursor.execute(
                    "SELECT id, from_agent, content, priority, timestamp FROM messages WHERE task_id = ? AND to_agent = 'orchestrator' AND read = 0 ORDER BY timestamp DESC",
                    (self.current_task_id,),
                )
    ...
    def _categorize_feedback(self, items: list[FeedbackItem]) -> list[FeedbackItem]:
        ...
        for i in range(0, len(uncategorized), 30):
            self._categorize_batch(uncategorized[i : i + 30])
    ...
    def _categorize_batch(self, batch: list[FeedbackItem]):
        index_snip = ""
        try:
            paths = [p for p in [getattr(it, "file_path", None) for it in batch] if p]
            index_snip = load_symbol_json_context(file_paths=paths or None, max_rows=40, label="Known symbols near feedback paths")
...
"""
        try:
            response = call_agent("prioritizer", prompt, self.current_task_id, model_override=_phase_model_override("batch"))
            if not response:
                return
            data = parse_json_response(response, expected_keys=["categorized"], agent_name="prioritizer/categorize")
            ...
            with get_db_connection() as conn:
                for item in categorized:
                    ...
                    if item_id_str.startswith("msg_"):
                        continue
                    raw_id = int(item_id_str.replace("fb_", "", 1)) if item_id_str.startswith("fb_") else int(item["id"])
                    conn.execute("UPDATE agent_feedback SET category = ? WHERE id = ?", (item["category"], raw_id))
        except Exception as e:
...
"""
    ...
    def _cross_category_ranking(self, by_category: dict[str, list[FeedbackItem]]) -> list[FeedbackItem]:
        ...
        for item in top_items:
            prompt += f"ID: {item.id} | Priority: {item.priority} | Category: {item.category} | Bias: {item.bias_multiplier}x | Score: {item.score}\n"
        prompt += """
...
"""
        try:
            response = call_agent("prioritizer", prompt, self.current_task_id, model_override=_phase_model_override("rank"))
            if not response:
                return self._top_by_score(all_items)
            data = parse_json_response(response, expected_keys=["top_suggestions"], agent_name="prioritizer/categorize")
